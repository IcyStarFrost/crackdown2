...
hook.Add( "Tick", "crackdown2_pickupsystem", function()
    ...
    for i = 1, #players do
        ...
        local near = CD2FindInSphere( ply:GetPos(), 100, function( ent ) return ( isphysics[ ent:GetClass() ] or ent.cd2_allowpickup ) and ent:GetNW2Int( "cd2_mass", math.huge ) < ply:GetMaxPickupWeight() and ply:Visible( ent ) and ent:SqrRangeTo( ply ) < ( 100 * 100 ) and IsValid( ent:GetPhysicsObject() ) and ent:GetPhysicsObject():IsMotionEnabled() end )
